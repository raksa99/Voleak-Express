...
class _LiveTrackingScreenState extends State<LiveTrackingScreen> {
  ...
  Future<LatLng?> _resolveCoordinates(String cityName) async {
    ...
    try {
      ...
      final response = await http.get(
        url,
        headers: {'User-Agent': 'VoleakExpress/1.0.0'},
      );
  ...
  Future<void> _updateRoutePath() async {
    // If the truck has departed and we have its live position, draw directions from the truck to the destination!
    // Otherwise, draw from the origin station to the destination.
    final startPos = (_tripStatus == 'in_progress' && _truckPosition != null)
        ? _truckPosition
        : (_originLatLng ?? _truckPosition);
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      if (mounted && data != null) {
        ...
        // Update route polyline to start from the truck's live position
        _updateRoutePath();

        // Move map to truck position if map is ready
        if (_truckPosition != null && _mapReady) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (_mapReady) {
              _mapController.move(_truckPosition!, 13);
            }
          });
        }
      } else if (mounted) {
    ...
    _pollingTimer = Timer.periodic(const Duration(seconds: 5), (_) async {
      ...
      try {
        ...
        if (data != null && mounted) {
          ...
          final oldTruckPosition = _truckPosition;
          ...
          // (e.g., scheduled→in_progress updates route from origin→dest to truck→dest)
          if (_truckPosition != oldTruckPosition || _tripStatus != oldTripStatus) {
            _updateRoutePath();
          }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            tooltip: _followTruck ? context.tr.liveTooltipFollowingTruck : context.tr.liveTooltipFollowTruck,
            onPressed: () {
              setState(() => _followTruck = !_followTruck);
              if (_followTruck && _truckPosition != null && _mapReady) {
                _mapController.move(_truckPosition!, _mapController.camera.zoom);
              }
            },
          ),
          ...
          : _errorMessage != null
              ...
              children: [
                ...
                FlutterMap(
                  ...
                  options: MapOptions(
                    initialCenter:
                        _truckPosition ?? _originLatLng ?? _defaultCenter,
                    initialZoom: 12,
                    onTap: (_, _) => setState(() => _followTruck = false),
                    onMapReady: () {
                      setState(() => _mapReady = true);
                      if (_truckPosition != null) {
                        _mapController.move(_truckPosition!, 12);
                      }
                    },
                  ),
                  children: [
                    ...
                    TileLayer(
                      ...
                      userAgentPackageName: 'com.example.voleak_express',
                      maxZoom: 19,
                    ),
                ...
                if (_truckPosition == null && _tripStatus == 'scheduled')
                  Center(
                    child: Container(
                ...
                // ── LOCATING TRUCK chip (in_progress but no GPS yet) ───
                if (_truckPosition == null && _tripStatus == 'in_progress')
                  Positioned(
                    bottom: 130,
                    left: 0,
                    right: 0,
                    child: Center(
                      child: _LocatingTruckChip(),
                    ),
                  ),


                // ── FOLLOW TRUCK button (bottom right) ─────────────────
                if (_truckPosition != null && !_followTruck)
                  Positioned(
                    bottom: 120,
                    right: 16,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          if (status == 'in_progress' && truckPosition != null)
            Row(
              children: [
                Container(
                  width: 8,
            ...
            children: [
              ...
              if (truckPosition != null)
                Column(
                  ...
                  children: [
                    Text(
                      context.tr.liveTruckLocation,
                      style: const TextStyle(fontSize: 11, color: Color(0xFF9CA3AF)),
                    ),
                    const SizedBox(height: 4),
                    Text(
                      truckPosition!.latitude.toStringAsFixed(4),
                      style: const TextStyle(
                        fontSize: 11,
                        color: Color(0xFF6B7280),
                    ...
                    Text(
                      truckPosition!.longitude.toStringAsFixed(4),
                      style: const TextStyle(
                        fontSize: 11,
                        color: Color(0xFF6B7280),
                        fontFamily: 'monospace',
...
class _LocatingTruckChip extends StatefulWidget {
  const _LocatingTruckChip();

  @override
  State<_LocatingTruckChip> createState() => _LocatingTruckChipState();
}

class _LocatingTruckChipState extends State<_LocatingTruckChip>
    with SingleTickerProviderStateMixin {
  late AnimationController _controller;
